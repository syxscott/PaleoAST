"""
================================================================================
PaleoAST Phase 5 - AST Source Code Auditor
================================================================================
    ...
    def to_markdown(self) -> str:
        ...
        if self.functions_missing_docstring:
            ...
            for func in self.functions_missing_docstring[:30]:
                ...
                lines.append(f"| {Path(func.name).parent if ':' not in str(func.lineno) else 'N/A'} | {loc} | {func.lineno} |")
        else:
...
class ASTAuditor:
    ...
    IGNORED_DIRS = {
        ...
        '.venv', 'build', 'dist', '*.egg-info', 'node_modules'
    }
    ...
    def audit_directory(
        ...
        for py_file in py_files:
            try:
                ...
                self._report.issues.append(AuditIssue(
        ...
        # 更新总函数数
        self._report.total_functions = sum(
            len(funcs) for funcs in self._functions_by_file.values()
        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                if complexity > self.COMPLEXITY_THRESHOLD:
                    func_name = node.name
                    self._report.complexity_warnings.append(
