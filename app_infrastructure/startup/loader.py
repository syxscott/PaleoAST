...
class StartupLoader:
    ...
    def __init__(self):
        ...
        self._current_progress = StartupProgress(
            phase=StartupPhase.INITIALIZATION, message="Initializing...", progress=0
        )
    ...
    def register_module(
        ...
        with self._lock:
            self._modules[name] = StartupModule(
                name=name, import_func=import_func, weight=weight, dependencies=dependencies or []
            )
            logger.debug(f"Registered module: {name} (weight: {weight})")

    def add_callback(self, callback: Callable[[StartupProgress], None]) -> None:
        """
        添加进度回调
    ...
    def start(self, splash_callback: Callable[[int, str], None] = None) -> dict[str, Any]:
        ...
        # 阶段1: 初始化
        self._emit_progress(
            StartupProgress(phase=StartupPhase.INITIALIZATION, message="Starting PaleoAST...", progress=0)
        )
        if splash_callback:
        ...
        # 阶段2: 依赖检查
        self._emit_progress(
            StartupProgress(phase=StartupPhase.DEPENDENCY_CHECK, message="Checking dependencies...", progress=10)
        )
        if splash_callback:
        ...
        # 阶段3: 配置加载
        self._emit_progress(
            StartupProgress(phase=StartupPhase.CONFIG_LOADING, message="Loading configuration...", progress=20)
        )
        if splash_callback:
        ...
        # 阶段4: 模块导入
        self._emit_progress(
            StartupProgress(phase=StartupPhase.MODULE_IMPORT, message="Importing modules...", progress=30)
        )
        if splash_callback:
        ...
        while remaining:
            ...
            # 找到没有未加载依赖的模块
            ready = [name for name in remaining if all(dep in loaded for dep in self._modules[name].dependencies)]
            ...
            for name in ready:
                ...
                self._emit_progress(
                    StartupProgress(phase=StartupPhase.MODULE_IMPORT, message=msg, progress=int(current_progress))
                )
                if splash_callback:
        ...
        # 阶段5: 数据加载
        self._emit_progress(StartupProgress(phase=StartupPhase.DATA_LOADING, message="Loading data...", progress=85))
        if splash_callback:
        ...
        # 阶段6: UI初始化
        self._emit_progress(
            StartupProgress(phase=StartupPhase.UI_INITIALIZATION, message="Initializing UI...", progress=90)
        )
        if splash_callback:
        ...
        # 完成
        self._emit_progress(StartupProgress(phase=StartupPhase.READY, message="Ready!", progress=100))
        if splash_callback:
...
class PaleoASTLoader(StartupLoader):
    ...
    def _register_paleoast_modules(self) -> None:
        """注册PaleoAST模块"""
        # 核心配置
        self.register_module("config", lambda: __import__("config"), weight=1)

        # 工具模块 (无依赖)
        self.register_module("utils", lambda: __import__("utils"), weight=1)

        # 数据模型 (无依赖)
        self.register_module("models", lambda: __import__("models"), weight=2, dependencies=["config"])

        # 统计引擎
        self.register_module("statistics", lambda: __import__("statistics"), weight=3, dependencies=["utils", "models"])

        # 形态测量
        self.register_module(
            "morphometrics", lambda: __import__("morphometrics"), weight=3, dependencies=["utils", "models"]
        )

        # 3D形态测量
        self.register_module(
            "morpho3d", lambda: __import__("morpho3d"), weight=3, dependencies=["morphometrics", "utils"]
        )

        # 古生态学
        self.register_module("ecology", lambda: __import__("ecology"), weight=3, dependencies=["utils", "models"])

        # 生物地层学
        self.register_module(
            "stratigraphy", lambda: __import__("stratigraphy"), weight=3, dependencies=["utils", "models"]
        )

        # 可视化
        self.register_module(
            "visualization", lambda: __import__("visualization"), weight=2, dependencies=["models", "statistics"]
        )
        ...
        # 视图 (最后加载)
        self.register_module(
            "views", lambda: __import__("views"), weight=2, dependencies=["controllers", "visualization"]
        )
