...
class TPSResult:
    ...
    deformation_grid: Optional[npt.NDArray]  # Grid of warped points
...
class TPSAnalyzer:
        ...
        with self._lock:
            ...
            if source.shape[0] != target.shape[0]:
                raise MorphometricsError(
                    ...
                    details={
                        "source_landmarks": source.shape[0],
                        "target_landmarks": target.shape[0]
                    }
                )
    ...
    def _validate_configuration(
        ...
        if config.ndim == 1:
            ...
            raise MorphometricsError(
                "Configuration must be 1D (flat) or 2D (n_landmarks, n_dims)"
            )
    ...
    def _warp_points(
        ...
        for p in range(n_points):
            ...
            distances = np.array([
                np.linalg.norm(point - lm) if np.linalg.norm(point - lm) > 0 else 1e-10
                for lm in source
            ])
    ...
    @property
    def last_result(self) -> Optional[TPSResult]:
