...
class RelativeWarpsAnalyzer:
        ...
        with self._lock:
            ...
            if aligned_configurations.ndim != 3:
                raise ComputationError(
                    "Aligned configurations must be 3D (n_specimens, n_landmarks, n_dims)"
                )
            ...
            flattened = aligned_configurations.reshape(
            ...
            try:
                U, singular_values, Vt = np.linalg.svd(
                    flattened_centered, full_matrices=False
                )
            except np.linalg.LinAlgError as e:
                raise ComputationError(
                    "SVD failed during Relative Warps analysis",
    ...
    def get_shape_at_warp(
        self,
        result: Optional[RelativeWarpsResult] = None,
        warp_number: int = 0,
        ...
        if warp_number >= result.n_components:
            raise ComputationError(
                f"Warp number {warp_number} exceeds available components"
            )
    ...
    @property
    def last_result(self) -> Optional[RelativeWarpsResult]:
