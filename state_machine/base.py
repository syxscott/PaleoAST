...
from typing import (
    ...
    Iterator, Generic, TypeVar, Protocol, Tuple, FrozenSet
)
...
class State:
    ...
    def __post_init__(self):
        ...
        if self.is_initial and self.is_accepting:
            logger.warning(
                f"State '{self.name}' is both initial and accepting. "
...
class StateMachine(ABC):
    ...
    def __init__(
        ...
        allow_non_determinism: bool = True
    ):
        """
        初始化状态机
    ...
    def add_state(
        ...
        **metadata: Any
        ...
        if is_initial and self._initial_state is not None:
            raise ValueError(
                f"Cannot add initial state '{name}': "
                f"initial state '{self._initial_state.name}' already exists"
            )
    ...
    def remove_state(self, state: State) -> bool:
        ...
        for sid in list(self._transitions.keys()):
            self._transitions[sid] = {
                t for t in self._transitions[sid]
    ...
    def _find_state_by_name(self, name: str) -> Optional[State]:
        """根据名称查找状态"""
    ...
    def add_transition(
        ...
        if existing is not None and not self._allow_non_determinism:
            raise ValueError(
                f"Deterministic conflict: multiple transitions from "
                f"'{source.name}' with symbol '{symbol}'"
            )
        ...
        self._logger.debug(
            f"Added transition: {source.name} --[{symbol}]--> {target.name}"
        )
        return transition
    ...
    def process(self, input_string: str) -> bool:
        ...
        for i, char in enumerate(input_string):
            ...
            self._logger.debug(
                f"After '{char}' at pos {i}: {[s.name for s in current_states]}"
            )
    ...
    def is_deterministic(self) -> bool:
        ...
        """
        for transitions in self._transitions.values():
            symbol_counts: Dict[Optional[str], int] = {}
    ...
    def is_complete(self) -> bool:
        ...
        for state in self._states.values():
            for symbol in symbols:
                has_transition = any(
                    t.symbol == symbol for t in self._transitions[state.state_id]
                )
    ...
    def get_live_states(self) -> Set[State]:
        ...
        reverse_graph: Dict[str, Set[str]] = {
            sid: set() for sid in self._states.keys()
        }
    ...
    def to_dot(self) -> str:
        ...
        for state in self._states.values():
            ...
            label = state.name
            if state.is_initial:
                lines.append(f'    "{state.state_id}" [label="{label}", shape={shape}, style=filled, fillcolor=lightgrey];')
            else:
