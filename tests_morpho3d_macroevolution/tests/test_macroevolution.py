...
from macroevolution.cohort import (
    CohortSurvivorshipAnalysis,
    analyze_cohort_survivorship
)
from macroevolution.fbd import (
    GillespieSimulator,
    FossilizedBirthDeathProcess,
    simulate_fbd_process
)
...
class TestCohortSurvivorshipRates(unittest.TestCase):
    ...
    def test_rates_calculation(self):
        """测试速率计算"""
        records = [
            (10.0, 0.0), (10.0, 0.0), (10.0, 0.0), (10.0, 0.0),  # 4个存活
            (8.0, 2.0), (8.0, 2.0),  # 2个死亡
        ]
        intervals = [(0, 10)]
