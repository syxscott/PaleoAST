...
from morpho3d.gpa3d import GPA3D, GPA3DResult, procrustes_distance_3d
...
class TestGPA3DBasics(unittest.TestCase):
    ...
    def test_gpa_identical_configs(self):
        ...
        # 质心大小应该相同
        self.assertTrue(
            np.allclose(result.centroid_sizes, result.centroid_sizes[0])
        )
...
class TestProcrustesDistance(unittest.TestCase):
    ...
    def test_rotation_only(self):
        ...
        d, R_recovered, s = procrustes_distance_3d(config1, config2)
...
class TestGPA3DChaos(unittest.TestCase):
    ...
    def test_extreme_scale_differences(self):
        """测试极端缩放差异"""
        configs = [
            np.random.randn(10, 3) * 1e-10,
            np.random.randn(10, 3) * 1e10
        ]
