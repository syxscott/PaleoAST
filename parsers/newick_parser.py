...
class TreeNode:
    ...
    def reroot(self, new_root_name: str) -> Optional['TreeNode']:
        """
        将树重新根植于指定节点
    ...
    def _find_by_name(self, name: str) -> Optional['TreeNode']:
        """递归查找指定名称的节点"""
        if self.name == name:
            return self
        for child in self.children:
    ...
    def _reroute(self, new_root: 'TreeNode') -> 'TreeNode':
        """重新路由树结构"""
        # 创建新根节点
        new_root_node = TreeNode(name=new_root.name)
        
        # 复制子树
        def copy_subtree(node: TreeNode, parent: Optional[TreeNode]) -> TreeNode:
            new_node = TreeNode(
                name=node.name,
...
class NewickTree:
    ...
    def get_lca(self, name1: str, name2: str) -> Optional[TreeNode]:
        """
        获取两个节点的最近公共祖先(LCA)
...
class NewickParser:
    ...
    def parse(self, newick_string: str) -> NewickTree:
        ...
        self._logger.info(
            f"Parsed Newick tree with {tree_obj.leaf_count} leaves"
        )
    ...
    def _parse_subtree(self) -> Optional[TreeNode]:
        """
        解析子树
    ...
    def _parse_number(self) -> Optional[float]:
        """
        解析数字 (枝长)
