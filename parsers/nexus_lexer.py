...
from typing import (
    Dict, Set, List, Optional, Iterator, Tuple
)
...
class NexusLexer(BaseLexer):
    ...
    def __init__(self):
        ...
        # NEXUS关键字集合 (大小写不敏感)
        self._keywords = {
            'BEGIN', 'END', 'TAXLABELS', 'CHARSTATELABELS',
            'DIMENSIONS', 'MATRIX', 'FORMAT', 'TREES', 'TREE',
            'LINK', 'SET', 'OPTIONS', 'UTAXAMATRIX', 'CHARGROUPS',
            'TITLE', 'NTAX', 'NCHAR', 'INTERLEAVE', 'DATATYPE',
            'MISSING', 'GAP', 'SYMBOLS', 'EQUATE', 'MATCHCHAR',
            'MISSING', 'NEWTAXA', 'NOLABELS', 'STATUS', 'TAXNAME'
        }
    ...
    def _init_rules(self) -> None:
        ...
        # 符号
        self.add_rule(
            NexusTokenType.LBRACKET,
            r'\[',
            priority=40
        )
        
        self.add_rule(
            NexusTokenType.RBRACKET,
            r'\]',
            priority=41
        )
        
        self.add_rule(
            NexusTokenType.LPAREN,
            r'\(',
            priority=42
        )
        
        self.add_rule(
            NexusTokenType.RPAREN,
            r'\)',
            priority=43
        )
        
        self.add_rule(
            NexusTokenType.COLON,
            r':',
            priority=50
        )
        
        self.add_rule(
            NexusTokenType.COMMA,
            r',',
            priority=51
        )
        
        self.add_rule(
            NexusTokenType.EQUALS,
            r'=',
            priority=52
        )
        
        self.add_rule(
            NexusTokenType.ASTERISK,
            r'\*',
            priority=53
        )
        
        self.add_rule(
            NexusTokenType.END_STATEMENT,
            r';',
            priority=60
        )
        ...
        while position < length:
            ...
            if match is not None:
                ...
                self._logger.warning(
                    f"Unknown character at line {self._line}, column {self._column}: '{char}'"
                )
                position += 1
                self._column += 1
    ...
    def _scan_comment(self, source: str, position: int) -> Optional[Tuple[Token, int]]:
        """
        扫描注释内容
        ...
        if self._in_nested_comment:
            ...
            return (
                Token(
                    type=NexusTokenType.NESTED_COMMENT,
                    value=value,
                    line=start_line,
                    column=start_column
                ),
    ...
    def _try_match(self, source: str, position: int) -> Optional[re.Match]:
        """
        尝试在当前位置匹配规则
