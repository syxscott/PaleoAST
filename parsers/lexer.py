...
class LexerError(Exception):
    ...
    def __str__(self) -> str:
        return f"LexerError at line {self.line}, column {self.column}: {self.message} (character: '{self.char}')"
...
class BaseLexer(ABC):
    ...
    def __init__(self, skip_whitespace: bool = True, skip_newlines: bool = False, case_sensitive: bool = True):
        """
        初始化词法分析器
    ...
    def add_rule(self, token_type: TokenType, pattern: str, priority: int = 100, skip: bool = False) -> LexerRule:
        ...
        rule = LexerRule(token_type=token_type, pattern=compiled, priority=priority, skip=skip)
    ...
    def tokenize(self, source: str) -> list[Token]:
        ...
        while position < length:
            ...
            if match is not None:
                ...
                self._logger.error(
                    f"LexerError at line {self._line}, column {self._column}: unexpected character '{char}'"
                )

                raise LexerError(f"Unexpected character: '{char}'", line=self._line, column=self._column, char=char)
    ...
    def _try_match(self, source: str, position: int) -> Match[str] | None:
        """
        尝试在当前位置匹配所有规则
        ...
        best_match: Match[str] | None = None
