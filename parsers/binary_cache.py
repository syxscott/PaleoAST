...
class BinaryCache:
    ...
    def load(self, filepath: str, use_mmap: bool = False) -> dict[str, Any] | None:
        """
        从二进制缓存加载矩阵
        ...
        try:
            with open(filepath, "rb") as f:
                ...
                # 读取矩阵数据（原始字节，可能已压缩）
                raw_matrix_bytes = f.read(header.matrix_size)

                # 恢复矩阵
                if header.flags & 0x01:
                    matrix_bytes = zlib.decompress(raw_matrix_bytes)
                else:
                    matrix_bytes = raw_matrix_bytes
                ...
                matrix = np.frombuffer(matrix_bytes, dtype=dtype_map.get(header.dtype, np.float64)).reshape(
                    header.nrow, header.ncol
                )
                ...
                crc_data = (
                    ...
                    + self._serialize_labels(col_labels)
                    + metadata_bytes
                )
                ...
                self._logger.info(f"Loaded matrix {header.nrow}x{header.ncol} from {filepath}")
    ...
    def save_mmap(
        self, filepath: str, matrix, row_labels: list[str] | None = None, col_labels: list[str] | None = None
    ) -> bool:
        """
        保存并创建内存映射文件
    ...
    def load_mmap(self, filepath: str) -> tuple[Any, mmap.mmap] | None:
        """
        加载内存映射文件
        ...
        try:
            ...
            matrix = (
                np.frombuffer(matrix_bytes, dtype=dtype_map.get(header.dtype, np.float64))
    ...
    def get_info(self, filepath: str) -> dict[str, Any] | None:
        """
        获取文件信息而不加载数据
...
def load_matrix(filepath: str) -> dict[str, Any] | None:
    """
    加载二进制缓存矩阵的便捷函数
