...
class ColumnMetadata:
    ...
    def __post_init__(self) -> None:
        ...
        # Validate data type
        valid_types = [
            DataType.NOMINAL, DataType.ORDINAL, DataType.CONTINUOUS,
...
class ColumnMetadataManager:
    ...
    def get_metadata(self, column_index: int) -> Optional[ColumnMetadata]:
        """
    ...
    def set_metadata(
        ...
        with self._lock:
            if column_index < 0 or column_index >= self._n_columns:
                raise IndexError(
                    f"Column index {column_index} out of range [0, {self._n_columns})"
                )
        ...
        with self._lock:
            ...
            if meta is None:
                meta = ColumnMetadata(
                    ...
                    name=self._column_labels[column_index]
                )
            
            new_meta = meta.with_updates(data_type=data_type)
            self._metadata[column_index] = new_meta
    
    def set_group(self, column_index: int, group: Optional[str]) -> None:
        """
    ...
    def __repr__(self) -> str:
        ...
        with self._lock:
            ...
            return (
                f"ColumnMetadataManager(n_columns={self._n_columns}, "
                f"n_groups={n_groups})"
            )
