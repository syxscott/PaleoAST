    ...
    def __post_init__(self) -> None:
        ...
        if not self._is_valid_hex_color(self.color):
            raise DataValidationError(
                ...
                details={"expected": "#RRGGBB or #RGB"}
            )
        
        # Validate marker
        if self.marker not in CHART_MARKERS:
            raise DataValidationError(
                f"Invalid marker: '{self.marker}'",
                details={"valid_markers": CHART_MARKERS}
            )
        
        # Validate size
        if self.size <= 0:
            raise DataValidationError(
                f"Marker size must be positive: '{self.size}'"
            )
...
class RowMetadataManager:
    ...
    def get_metadata(self, row_index: int) -> Optional[RowMetadata]:
        """
    ...
    def set_metadata(self, row_index: int, metadata: RowMetadata) -> None:
        ...
        with self._lock:
            if row_index < 0 or row_index >= self._n_rows:
                raise IndexError(
                    f"Row index {row_index} out of range [0, {self._n_rows})"
                )
    ...
    def set_group(self, row_index: int, group: Optional[str]) -> None:
        """
    ...
    def set_age(self, row_index: int, age: Optional[float]) -> None:
        """
    ...
    def set_location(self, row_index: int, location: Optional[str]) -> None:
        """
    ...
    def get_all_groups(self) -> List[str]:
        ...
        with self._lock:
            groups = set(
                meta.group for meta in self._metadata.values()
    ...
    def get_group_sizes(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            return {
                group: len(self.get_rows_by_group(group))
                for group in groups
            }
    ...
    def get_row_labels(self) -> List[str]:
        ...
        with self._lock:
            return [
                self._metadata[i].label if i in self._metadata
                else self._row_labels[i]
                for i in range(self._n_rows)
            ]
    
    def get_groups(self) -> Dict[int, Optional[str]]:
        """
    ...
    def __repr__(self) -> str:
        with self._lock:
            ...
            return (
                f"RowMetadataManager(n_rows={self._n_rows}, "
                f"n_groups={n_groups})"
            )
