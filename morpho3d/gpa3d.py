...
class GPA3DResult:
    ...
    def get_shape_coordinates(self) -> np.ndarray:
        ...
        """
        return np.array([
            config.flatten() for config in self.aligned_configs
        ])
...
class GPA3D:
    ...
    def analyze(
        ...
        self._logger.info(
            f"Starting 3D GPA with {n_samples} configs, "
        ...
        for iteration in range(self._max_iter):
            ...
            if prev_mean is not None:
                ...
                if self._verbose:
                    self._logger.debug(
                        f"Iteration {iteration}: diff = {diff:.2e}, "
                        f"diff_norm = {diff_norm:.2e}"
                    )
        ...
        final_spread = np.sum([
            np.linalg.norm(config - final_mean)**2 
        ...
        self._logger.info(
            f"GPA complete: {n_iterations} iterations, "
            f"final spread = {final_spread:.4f}"
        )
    ...
    def partial_gpa(
        ...
        # 提取固定点
        fixed_configs = [
            config[fixed_indices] for config in configs
        ]
