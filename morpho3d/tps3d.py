...
class TPS3D:
    ...
    def fit(self, source: np.ndarray, target: np.ndarray) -> TPS3D:
        """
        拟合TPS变换
    ...
    def _compute_kernel_matrix(self, points1: np.ndarray, points2: np.ndarray) -> np.ndarray:
        """
        计算核函数矩阵
    ...
    def create_deformation_grid(
        self, grid_range: tuple[float, float, float, float, float, float], resolution: tuple[int, int, int]
