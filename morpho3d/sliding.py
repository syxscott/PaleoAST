...
class SemiLandmarkSlider:
    ...
    def __init__(
        ...
        self._semi_indices: Optional[np.ndarray] = None
        self._curve_topology: str = 'open'
        self._surface_mesh: Optional[np.ndarray] = None
    ...
    def slide(
        ...
        for iteration in range(self._max_iterations):
            ...
            if prev_mean is not None:
                # 计算半标志点区域的变化
                semi_mean_diff = np.linalg.norm(
                    current_mean[self._semi_indices] - 
                    prev_mean[self._semi_indices]
                )
                
                if self._verbose:
                    self._logger.debug(
                        f"Iteration {iteration}: semi-diff = {semi_mean_diff:.2e}"
                    )
            ...
            for i in range(n_samples):
                if self._surface_mesh is not None:
                    ...
                    new_semi = self._slide_surface_points(
                        current_configs[i],
                        gpa_result.aligned_configs[i],
                        current_mean
                    )
                else:
                    # 曲线滑动
                    new_semi = self._slide_curve_points(
                        current_configs[i],
                        gpa_result.aligned_configs[i],
                        current_mean
                    )
        ...
        final_be = self._compute_bending_energy(
            current_configs,
            final_gpa.mean_config
        )
        
        self._logger.info(
            f"Sliding complete: {n_iterations} iterations, "
            f"final bending energy = {final_be:.4f}"
        )
    ...
    def _gpa_with_fixed_landmarks(
        ...
        # 提取固定界标
        fixed_configs = [
            config[self._fixed_indices] for config in configs
        ]
    ...
    def _slide_curve_points(
        ...
        # 获取半标志点
        semi_original = original[self._semi_indices]
        ...
        # 计算切向量
        tangents = self._compute_curve_tangents(
            semi_mean,
            positions=semi_positions,
            n_total=n_total
        )
    ...
    def _compute_bending_energy(
        ...
        for config in configs:
            ...
            tps.fit(
                mean_shape[self._fixed_indices],
                config[self._fixed_indices]
            )
            total_be += tps._compute_bending_energy(
                tps._compute_kernel_matrix(
                    mean_shape[self._semi_indices],
                    mean_shape[self._semi_indices]
                )
            )
