...
from .fitch import FitchAlgorithm, FitchResult
...
class SearchResult:
    ...
    @property
    def consensus_tree(self) -> Optional[PhyloTree]:
        """如果有多棵等长树，返回一致性树"""
        if len(self.all_trees) <= 1:
            return None
        # 需要StrictConsensusTree类
        return None
...
class NNIOperation(TreeOperation):
    ...
    def __init__(
        ...
        super().__init__(
            f"NNI: swap option {swap_option} on edge ({edge_node1.name}, {edge_node2.name})"
        )
    ...
    def _deep_copy_tree(self, root: PhyloNode) -> PhyloNode:
        """深度拷贝树"""
        def copy_node(node: PhyloNode, parent: Optional[PhyloNode]) -> PhyloNode:
            new_node = PhyloNode(
                name=node.name,
...
class TBROperation(TreeOperation):
    ...
    步骤:
        1. 选择一条边，将其移除，将树分为两部分
        2. 在每部分中选择一个节点
        3. 创建新边连接两点
    
    TBR比NNI产生更多的邻居，但搜索更彻底。
    """
...
class HeuristicSearch:
    ...
    def __init__(
        ...
        # 搜索状态
        self._current_tree: Optional[PhyloTree] = None
        self._current_score: float = float('inf')
        self._best_tree: Optional[PhyloTree] = None
    ...
    def search(
        ...
        initial_tree: Optional[PhyloTree] = None
    ) -> SearchResult:
        """
        执行启发式搜索
        ...
        self._logger.info(
            f"Starting heuristic search with initial score: {self._current_score}"
        )
        ...
        for iteration in range(self._max_iterations):
            ...
            if self._should_accept(best_neighbor_score):
                ...
                if best_neighbor_score < self._best_score:
                    ...
                    self._logger.info(
                        f"Iteration {iteration}: New best score = {best_neighbor_score}"
                    )
    ...
    def _collect_internal_edges(
        self,
        node: PhyloNode
    ) -> List[Tuple[PhyloNode, PhyloNode]]:
        """
        收集所有内部边
    ...
    def _should_accept(self, new_score: float) -> bool:
        ...
        # 接受次优解的概率
        probability = min(1.0, self._acceptance_prob)
