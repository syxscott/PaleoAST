...
def ensure_matrix(
    data: Union[npt.NDArray, list, tuple], dtype: np.dtype | None = None, copy: bool = False
) -> npt.NDArray:
    """
    ...
    if isinstance(data, np.ndarray):
        ...
        raise MatrixDimensionError(
            f"Cannot convert {type(data)} to numpy array", details={"input_type": str(type(data))}
        )
...
def validate_matrix_shape(
    ...
    if not isinstance(matrix, np.ndarray):
        raise MatrixDimensionError("Input must be a numpy ndarray", details={"input_type": str(type(matrix))})

    if matrix.ndim != 2:
        raise MatrixDimensionError(
            f"Expected 2D array, got {matrix.ndim}D",
            details={"ndim": matrix.ndim, "shape": matrix.shape},
        )
    ...
    if exact_shape is not None:
        ...
        if n_rows != expected_rows or n_cols != expected_cols:
            raise MatrixDimensionError(
                f"Matrix shape must be {exact_shape}", details={"expected": exact_shape, "actual": matrix.shape}
            )

    # Check row constraints
    if min_rows is not None and n_rows < min_rows:
        raise MatrixDimensionError(
            f"Matrix must have at least {min_rows} rows", details={"minimum_rows": min_rows, "actual_rows": n_rows}
        )

    if max_rows is not None and n_rows > max_rows:
        raise MatrixDimensionError(
            f"Matrix cannot have more than {max_rows} rows", details={"maximum_rows": max_rows, "actual_rows": n_rows}
        )

    # Check column constraints
    if min_cols is not None and n_cols < min_cols:
        raise MatrixDimensionError(
            f"Matrix must have at least {min_cols} columns", details={"minimum_cols": min_cols, "actual_cols": n_cols}
        )

    if max_cols is not None and n_cols > max_cols:
        raise MatrixDimensionError(
            f"Matrix cannot have more than {max_cols} columns",
            details={"maximum_cols": max_cols, "actual_cols": n_cols},
        )
...
def standardize_matrix(matrix: npt.NDArray, axis: int = 0, ddof: int = 1) -> npt.NDArray:
    """
...
def covariance_matrix(matrix: npt.NDArray, rowvar: bool = False, ddof: int = 1, bias: bool = False) -> npt.NDArray:
    """
...
def correlation_matrix(matrix: npt.NDArray, method: str = "pearson") -> npt.NDArray:
    """
    ...
    if method.lower() != "pearson":
        raise NotImplementedError(f"Correlation method '{method}' not yet implemented")
...
def mahalanobis_distance(x: npt.NDArray, mean: npt.NDArray, cov: npt.NDArray, inverted: bool = False) -> float:
    """
    ...
    if x.shape[1] != mean.shape[0]:
        raise MatrixDimensionError(
            "Point dimension must match mean dimension", details={"point_dim": x.shape[1], "mean_dim": mean.shape[0]}
        )

    if cov.shape[0] != cov.shape[1]:
        raise MatrixDimensionError("Covariance matrix must be square", details={"cov_shape": cov.shape})

    if cov.shape[0] != mean.shape[0]:
        raise MatrixDimensionError(
            "Covariance matrix dimension must match mean dimension",
            details={"cov_dim": cov.shape[0], "mean_dim": mean.shape[0]},
        )
    ...
    if not inverted:
        try:
            ...
            raise ComputationError("Covariance matrix is singular or near-singular", details={"cov_shape": cov.shape})
    else:
        precision = cov
...
def euclidean_distance_matrix(matrix: npt.NDArray, squared: bool = False) -> npt.NDArray:
    """
...
def pairwise_distances(
    matrix1: npt.NDArray, matrix2: npt.NDArray | None = None, metric: str = "euclidean", **kwargs
) -> npt.NDArray:
    """
    ...
    if matrix2 is not None:
        ...
        if matrix1.shape[1] != matrix2.shape[1]:
            ...
            raise MatrixDimensionError(
                ...
                details={"matrix1_cols": matrix1.shape[1], "matrix2_cols": matrix2.shape[1]},
            )
...
def _compute_self_distances(matrix: npt.NDArray, metric: str, **kwargs) -> npt.NDArray:
    """
...
def _compute_cross_distances(matrix1: npt.NDArray, matrix2: npt.NDArray, metric: str, **kwargs) -> npt.NDArray:
    """
...
def _bray_curtis_cross_distance(matrix1: npt.NDArray, matrix2: npt.NDArray) -> npt.NDArray:
    """
...
def _jaccard_cross_distance(matrix1: npt.NDArray, matrix2: npt.NDArray) -> npt.NDArray:
    """
