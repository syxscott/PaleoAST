    ...
    if not allow_nan and nan_count > 0:
        ...
        raise DataValidationError(
            ...
            details={
                ...
                "positions": list(zip(nan_positions[0][:5], nan_positions[1][:5]))
            }
        )
    ...
    if not allow_inf and inf_count > 0:
        ...
        raise DataValidationError(
            ...
            details={
                ...
                "positions": list(zip(inf_positions[0][:5], inf_positions[1][:5]))
            }
        )
...
def validate_column_metadata(
    ...
    valid_markers: Optional[List[str]] = None
) -> bool:
    """
    ...
    for col_idx, col_meta in metadata.items():
        ...
        if not isinstance(col_idx, int):
            raise DataValidationError(
                f"Column index must be integer, got {type(col_idx)}",
                details={"column_index": str(col_idx)}
            )
        ...
        if not isinstance(col_meta, dict):
            raise DataValidationError(
                f"Column {col_idx} metadata must be a dictionary",
                details={"type": str(type(col_meta))}
            )
...
def validate_row_labels(
    ...
    if none_count > 0 and not allow_none:
        raise DataValidationError(
            f"Row labels contain {none_count} None/empty value(s)",
            details={"none_count": none_count}
        )
    
    return label_list


def validate_distance_metric(
    metric: str,
...
def check_missing_values(
    ...
    if report_positions and total_nan > 0:
        nan_positions = np.where(nan_mask)
        positions = list(zip(
            nan_positions[0].tolist()[:max_positions],
            nan_positions[1].tolist()[:max_positions]
        ))
...
def check_infinite_values(matrix: npt.NDArray) -> Dict[str, Any]:
    ...
    if total_inf > 0:
        ...
        result["positions"] = list(zip(
            positions[0].tolist()[:10],
            positions[1].tolist()[:10]
        ))
