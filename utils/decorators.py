...
def memoize(func: Callable[P, T]) -> Callable[P, T]:
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        key_args = tuple(
            tuple(arg) if isinstance(arg, (list, np.ndarray)) else arg
            for arg in args
        )
        key_kwargs = tuple(
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
            ...
            try:
                ...
                if success:
                    log.log(
                        level,
                        f"{f.__name__} completed in {duration_str}"
                    )
                else:
                    log.log(
                        level,
                        f"{f.__name__} failed after {duration_str}: {error}"
                    )
            ...
            for param_name, rules in validators.items():
                ...
                if "ndim" in rules and isinstance(value, (np.ndarray,)):
                    if value.ndim != rules["ndim"]:
                        raise ValueError(
                            f"Parameter '{param_name}' must have {rules['ndim']} "
                ...
                if "shape" in rules and isinstance(value, np.ndarray):
                    ...
                    for i, (exp, actual) in enumerate(zip(expected_shape, value.shape)):
                        if exp is not None and exp != actual:
                            raise ValueError(
                                f"Parameter '{param_name}' dimension {i}: "
        ...
        try:
            ...
            if app is None:
                raise RuntimeError(
                    ...
                    "Create QApplication before calling this function."
                )
        except ImportError:
            raise ImportError(
                "PyQt6 is required for this function. "
