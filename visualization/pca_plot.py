...
class PCAPlotter:
    ...
    def plot_scores(
        ...
        if groups is not None:
            ...
            colors = get_color_scheme(len(unique_groups))
    ...
    def plot_biplot(
        ...
        pc2: int = 1,
        labels: Optional[List[str]] = None,
        loading_labels: Optional[List[str]] = None,
    ...
    def _add_loadings(
        ...
        scale: float,
        mean_vector: Optional[npt.NDArray] = None
    ) -> None:
        """Add loading vectors to plot."""
        origin = np.array([0, 0])
        max_score = np.max(np.abs(loadings)) * scale
