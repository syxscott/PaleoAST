...
def compute_sample_based_rarefaction(
    ...
    """
    n_samples, n_species = occurrence_matrix.shape
    ...
    for i in range(n_samples):
        ...
        for j, k in enumerate(sample_k):
            ...
            expected_species[j] = np.sum(
                1 - np.array([
                    _combinations(n_occurrences - int(occ), k) / _combinations(n_occurrences, k)
                    for occ in occurrences
                ])
            )
        
        results.append(RarefactionResult(
...
class RarefactionAnalyzer:
    ...
    def __init__(self) -> None:
        ...
        self._last_result: Optional[RarefactionResult] = None
    ...
    @property
    def last_result(self) -> Optional[RarefactionResult]:
