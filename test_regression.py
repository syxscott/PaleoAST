...
def t4():
    from reporting import LatexCompiler, ReportBuilder
...
def t5():
    from state_machine import DFA, NFA, LexerTokenizer, RegexCompiler, Token, TokenType
...
def t_upgma():
    ...
    dm = np.array(
        [[0, 5, 9, 9, 8], [5, 0, 10, 10, 9], [9, 10, 0, 8, 7], [9, 10, 8, 0, 3], [8, 9, 7, 3, 0]], dtype=float
    )
    labels = ["A", "B", "C", "D", "E"]
...
def t_matops():
    from utils.matrix_ops import center_matrix, ensure_matrix, pairwise_distances
